...
def sort_all(table):
    ...
    for r in all_jobs:
        table.insert(
            "", i, text="", values=(r[1], r[2], r[3], r[4], r[5], r[6])
        )
        i += 1
...
def create():
    ...
    role_l = Label(
        tab, text="Role :", font=("normal", 18, "bold"), bg="#FFFFFF"
    )
    role_l.grid(row=0, column=0, pady=10, padx=10)
    type_l = Label(
        tab, text="Type :", font=("normal", 18, "bold"), bg="#FFFFFF"
    )
    type_l.grid(row=1, column=0, pady=10, padx=10)
    qual_l = Label(
        tab, text="Qualification :", font=("normal", 18, "bold"), bg="#FFFFFF"
    )
    qual_l.grid(row=2, column=0, pady=10, padx=10)
    exp_l = Label(
        tab, text="Experience :", font=("normal", 18, "bold"), bg="#FFFFFF"
    )
    exp_l.grid(row=3, column=0, pady=10, padx=10)
    sal_l = Label(
        tab, text="Salary :", font=("normal", 18, "bold"), bg="#FFFFFF"
    )
    sal_l.grid(row=4, column=0, pady=10, padx=10)

    # Entries
    style = ttk.Style(tab)
    style.configure("TCombobox", background="white", foreground="#696969")

    role = Entry(tab, placeholder="Enter Job Role")
    role.grid(row=0, column=1, pady=10, padx=10)
    jtype = ttk.Combobox(tab, font=("normal", 18), width=23, state="readonly")
...
def app():
    ...
    search = Button(
        ...
        bg="#00b9ed",
        fg="#ffffff",
        command=lambda: sort_applicants(table),
    )
